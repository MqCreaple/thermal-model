...
use thermal_model::{PlotOptions, VisualizerOptions, Visualizer};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let model = Model3::<MoleculeTypes, 1>::construct(
        WIDTH, HEIGHT, NUM_MOLECULES,
        |i| {
            if i < NUM_MOLECULES / 2 {
                Molecule {
                    pos: Vec2::new(rng.gen_range(0.0..(WIDTH / 2.0)), rng.gen_range(0.0..HEIGHT)),
                    vel: sample_rand_velocity(&mut rng, 1.0),
                    mol_type: MoleculeTypes,
                }
            } else {
                Molecule {
                    pos: Vec2::new(rng.gen_range((WIDTH / 2.0)..WIDTH), rng.gen_range(0.0..HEIGHT)),
    ...
    let visualizer_options = VisualizerOptions::<Model3::<MoleculeTypes, 1>, Vec2> {
        ...
        state_quantities: vec![
            ...
            (|model| {
                ...
                if count == 0 { None } else { Some(total_energy / count as f32) }
        ...
        grid_color: |quantity, model, x_count, y_count| {
            ...
            let value = f32::min(1.0, 0.5 * (count as usize * x_count * y_count) as f32 / model.num_molecule() as f32);
