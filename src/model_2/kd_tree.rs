...
impl<'a, E: PositionKey + Clone> Node<'a, E> {
        ...
        if elems.len() <= max_size {
            ...
            Self {
                ...
                inner: NodeInner::XNode {
                    ...
                    right: Box::new(Self::new(
                        unsafe { from_raw_parts_mut(mid, right.len() + 1) },
                        max_size,
                        KDNodeDivideBy::Y,
                    )),
                },
            ...
            Self {
                ...
                inner: NodeInner::YNode {
                    ...
                    // TODO: unsafe combination of slices
                    down: Box::new(Self::new(
                        unsafe { from_raw_parts_mut(mid, down.len() + 1) },
                        max_size,
                        KDNodeDivideBy::X,
                    )),
                },
    ...
    pub fn query_circle(&self, center: Vec2, radius: f32) -> Option<Vec<&E>> {
        if self.x_bound.start > center.x + radius
            || self.x_bound.end < center.x - radius
            || self.y_bound.start > center.y + radius
            || self.y_bound.end < center.y - radius
        {
            return None;
        }
        match &self.inner {
            NodeInner::Leaf(elems) => Some(
                elems
                    .iter()
                    .filter_map(|elem| {
